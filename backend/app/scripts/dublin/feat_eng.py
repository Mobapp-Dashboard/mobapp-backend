...
class Feat_eng():
    ...
    def _discretize_time(data):
        ...
        data['day_moment'].where(conditions[0], 'MADRUGADA', inplace=True)
        data['day_moment'].where(conditions[1], 'MANHÃ', inplace=True)
        data['day_moment'].where(conditions[2], 'TARDE', inplace=True)
        data['day_moment'].where(conditions[3], 'NOITE', inplace=True)
        return data
