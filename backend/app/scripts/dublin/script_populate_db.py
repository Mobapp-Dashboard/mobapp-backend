...
from config import (engine, features, trajetoria, list_ints, PATH_LOCAL,
...
def save_trajectories_metadata(data, traj_ID_list):
    ...
    data.columns = [*traj_ID_list, "trajectory_size"]
    types_psql = dict(zip(traj_ID_list, PSQL_TYPES_META_TRAJ))
    save_in_db(data, TABLEMETA , types_psql)
...
def save_trajectories_points(data):
    ...
    save_in_db(data, TABLEPOINTS, psql_types_pt)
...
def make_ID(data):
    ...
    query_result = conn.execute( f"SELECT * FROM {TABLEMETA} WHERE {query};").fetchall()
    try:
...
def transform_columns(data):
    data = data[['trajectory_id', '_timestamp', 'lat', 'lng', 'lat_mercator',
                 ...
                 'time_old_point', 'cum_dist', 'cum_time', 'bearing', 'year', 'month', 'day', 'hour',
                 'minute',   "first_lat", "first_lng", "last_lat", "last_lng", 
                 "first_instant", "last_instant", "day_moment"]]
    ...
    data.columns = ['trajectory_id', 'instant', 'lat', 'lng',
                    ...
                    "last_lng", "first_instant", "last_instant", "day_moment"]
    return data
