...
class CRUDDublinPoints(CRUDBase[DublinPoints, DublinPointsCreate, DublinPointsUpdate]):
    ...
    def get_traj_by_multiparam(
        ...
        if "journey_id" in kwargs.keys():
            journey_subquery = (
                db
                .query(DublinMeta.trajectory_id)
                .distinct()
...
#                .limit(100)
        ...
        if "turno" in kwargs.keys():
            turno_subquery = (
                db
                .query(DublinPoints.trajectory_id)
                .distinct()
                .filter(
                    (DublinPoints.hour >= kwargs["turno"][0])
                    & (DublinPoints.hour <= kwargs["turno"][1])
                )
#                .limit(100)
                .subquery()
            )
